...
gulp.task('ammap-export:styles', function () {
  return gulp.src(config.src.styles + '/ammap-export.scss')
    .pipe(when(!production, $.sourcemaps.init()))
    .pipe($.sass({importer: moduleImporter()}))
    .on('error', $.sass.logError)
    .pipe($.autoprefixer({browsers: ['last 2 versions', 'iOS 8']}))
...
gulp.task('styles', gulp.parallel('main:styles', 'font:styles', 'vendor:styles', 'ammap-export:styles'))
