...
gulp.task('ammap-lib:scripts', () => {
  return browserify({
    entries: config.src.scripts + '/ammap-lib.js',
    debug: !production
  })
    .external(vendors)
    .transform(transform)
    .bundle().on('error', config.onError)
    .pipe(source('ammap-lib.js'))
    .pipe(buffer())
    .pipe(when(!production, $.sourcemaps.init({ loadMaps: true })))
    .pipe(when(!production, $.sourcemaps.write()))
    // All production stuff here
...
gulp.task('scripts', gulp.parallel('vendor:scripts', 'main:scripts', 'ammap-lib:scripts'))
